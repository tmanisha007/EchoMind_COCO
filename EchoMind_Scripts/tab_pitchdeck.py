...
def _build_html(total_calls, total_segs, total_km, avg_sent,
    ...
    km_html = "".join([
        f"<div class='km-badge'>{{'Frustration':'😤','Escalation_Request':'🔺','Complaint':'😠','Buying_Signal':'💰','Resolution':'✅','Positive_Feedback':'😊','Objection':'🚧'}.get('{t}','📌')} {t}</div>"
        for t in km_types
    ]) if km_types else "<div class='km-badge'>⭐ No key moments yet</div>"

    # Replace Python f-string dict lookup with static mapping
    ...
    <div style="text-align:center; max-width:900px; width:100%;">
      ...
      <div style="margin-top:28px;" class="card">
        ...
        <div style="display:flex; align-items:center; gap:0; flex-wrap:wrap; justify-content:center;">
          {"".join([f'<div style="display:flex;align-items:center;gap:0;"><div style="background:rgba(59,130,246,0.15);border:1px solid rgba(59,130,246,0.3);border-radius:8px;padding:8px 14px;font-size:13px;font-weight:600;">{s}</div><div style="color:#64748b;padding:0 6px;font-size:18px;">→</div></div>' for s in ["Upload","Transcribe","Diarise","Sentiment","Topics","KPIs","Coaching"]])[:-56]}
