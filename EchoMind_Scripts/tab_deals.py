    ...
    with sub3:
        ...
        if len(calls_df2) == 0:
            ...
            if st.button("🧠 Generate Win/Loss AI Report", key="wl_report_btn", type="primary"):
                res_lead  = f"{resolved['LEAD'].mean():.0f}"   if len(resolved)   > 0 else "0"
                res_sent  = f"{resolved['SENT'].mean():.2f}"   if len(resolved)   > 0 else "0"
                unr_lead  = f"{unresolved['LEAD'].mean():.0f}" if len(unresolved) > 0 else "0"
                unr_sent  = f"{unresolved['SENT'].mean():.2f}" if len(unresolved) > 0 else "0"
                wl_data = f"""
Resolved conversations: {len(resolved)}, avg lead score: {res_lead}, avg sentiment: {res_sent}
Unresolved: {len(unresolved)}, avg lead: {unr_lead}, avg sentiment: {unr_sent}
