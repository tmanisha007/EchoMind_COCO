...
def render(session):
    ...
    with sub1:
        ...
        # chat_input must come before history display
    ...
    with sub3:
        ...
        # Only keep defaults that are actually in the preset_tags list (guards against stale/custom tags)
        valid_defaults = [t for t in current_tags if t in preset_tags]
        selected_tags = st.multiselect("Apply tags", preset_tags, default=valid_defaults, key=f"tags_{sel_tag}")
