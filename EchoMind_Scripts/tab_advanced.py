    ...
    with sub2:
        ...
        if len(lb) == 0:
            ...
            for rank, (_, row) in enumerate(lb.iterrows(), 1):
                ...
                score_label = "🟢" if score >= 70 else "🟡" if score >= 40 else "🔴"
                ...
                with st.container(border=True):
                    ...
                    with c_left:
                        ...
                        st.markdown(f"{medal} **{row['CALL_ID']}**{active_badge}")
                        st.caption(f"⏱ {dur_m}m {dur_s}s  ·  {res_icon} {row['RESOLUTION']}  ·  {csat_icon} {row['CSAT']}")
                    with c_right:
                        st.metric(f"{score_label} Lead Score", f"{score}/100")
